...
export async function hydrateSessionFromAccountHolderId(accountHolderId, tracker) {
  ...
  // Best-effort lookup of the paymentProcessing business line for this LE, and any
  // store that references it. Failures here are non-fatal — login must not depend on them.
  let paymentsBusinessLineId = "";
  let storeId = "";
  let storeReference = "";
  try {
    const businessLinesResponse = await adyenLemRequest(
      `/legalEntities/${encodeURIComponent(legalEntityId)}/businessLines`,
      "GET"
    );
    ...
    console.error("[hydrate] business line lookup failed:", blError?.message || blError);
  }
    ...
    try {
      ...
      if (merchantAccount) {
        ...
        const matchingStore = stores.find((s) =>
          Array.isArray(s?.businessLineIds) && s.businessLineIds.includes(paymentsBusinessLineId)
        );
        storeId = String(matchingStore?.id || "").trim();
        storeReference = String(matchingStore?.reference || "").trim();
      }
    } catch (storeError) {
      console.error("[hydrate] store lookup failed:", storeError?.message || storeError);
    }
  }
