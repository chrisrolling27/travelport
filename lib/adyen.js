...
// Dedicated Checkout helper for the Drop-in flow. Uses an entirely separate
// merchant account / API key so the issued-card test rails don't collide with
// the Acquire-Flight (platform-acquiring) merchant.
export async function adyenDropInCheckoutRequest(path, method = "GET", body) {
  return request(
    ADYEN_CHECKOUT_URL,
    path,
    method,
    body,
    process.env.DROP_IN_ADYEN_PAYMENTS_API_KEY
  );
}
